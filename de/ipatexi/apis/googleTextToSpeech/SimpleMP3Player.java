...
public class SimpleMP3Player implements GoogleTextToSpeechObserver {
	...
	public SimpleMP3Player(){
		...
		//If you run a proper javafx application those nasty workarounds are not necessary
	...
	/*Id to match separate mp3 files which belong together. If this is connected to multiple 
	 * conversions we ignore every other file which is not part of the one first recieved file.
	 */
	...
	public void fileDownloaded(File f, int id) {
		//We will only take care of the most current id and discard everything else for now
		if(id == currentId){
